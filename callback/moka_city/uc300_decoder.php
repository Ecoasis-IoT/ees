...
if (isset($object['modbus_chn_2'])) {
    ...
    if ($total_active_energy > 0) {
        ...
        // MUST be scoped to this meter: if an LV gateway webhook also writes
        // tbl_hourly_prod (meter_id 1..8), an unscoped query would pick up those
        // rows and make the main meter think it had already caught up, silently
        // dropping its own hourly production rows.
