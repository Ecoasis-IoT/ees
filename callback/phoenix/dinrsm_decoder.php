...
$get_meters = "SELECT `id`, `meter_name`, `header` from tbl_meters WHERE `header` = $header";
$result = mysqli_query($phoenix_link, $get_meters);
...
if(mysqli_num_rows($result) > 0 and $code == "03"){
    ...
    if ((int)$header >= 0 and (int)$header <= 24){
        ...
        $query = "INSERT INTO `tbl_sub_meters`(`date`, `meter_id`, `meter_name`, `active_power`, `power_factor`, `total_active_energy`) VALUES ('" . $round_date . "', '" . $meter['id'] . "', '" . $meter['meter_name'] . "', " . round($active_power, 5) . ", " . round($pf, 5) . ", " . round($total_active_energy, 5) . ")";
            ...
            $hist_result = mysqli_query($phoenix_link, $hist_query);
...
if(mysqli_query($phoenix_link, $query)){
    //Do Nothing! All Good.
}
else{
