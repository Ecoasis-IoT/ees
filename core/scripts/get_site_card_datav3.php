...
$active_power = "(SELECT
                    ...
                    FROM
                        `plant_active_power`
                    WHERE
                        DATE(DATE) = DATE('$timenow') and `meter_id` = 100
                    ORDER BY
                        DATE
                    DESC
                    LIMIT 1)
                    ...
                    FROM
                        `plant_active_power`
                    WHERE
                        DATE(DATE) = DATE('$timenow') and `meter_id` = 101
                    ORDER BY
                        DATE
                    DESC
                    LIMIT 1)
                    ...
                    FROM
                        `plant_active_power`
                    WHERE
                        DATE(DATE) = DATE('$timenow') and `meter_id` = 102
                    ORDER BY
                        DATE
                    DESC
                    LIMIT 1)";
...
$site_power = mysqli_fetch_all($result, MYSQLI_ASSOC);
...
// echo json_encode(array("active_power1"=>round($site_power[0]['active_power'],2),"active_power2"=>round($site_power[1]['active_power'],2),"active_power3"=>round($site_power[2]['active_power'],2), "daily_prod"=>round($site_daily['daily'],2), "monthly_prod"=>round($site_monthly['monthly'],2), "yearly_prod"=>round($site_yearly['yearly'],2), "avg_irr" => round($avg_irradiance['avg'], 2), "sun_hours" => $sun_hours['minutes']));
