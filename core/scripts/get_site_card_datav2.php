...
try {
    $ap = $pdo->prepare(
        "(SELECT meter_id, active_power FROM plant_active_power
          WHERE DATE(`date`) = DATE(:now1) AND meter_id = 100 ORDER BY `date` DESC LIMIT 1)
         UNION ALL
         (SELECT meter_id, active_power FROM plant_active_power
          WHERE DATE(`date`) = DATE(:now2) AND meter_id = 101 ORDER BY `date` DESC LIMIT 1)"
    );
    $ap->execute([':now1' => $timenow, ':now2' => $timenow]);
